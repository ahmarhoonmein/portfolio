...
(function () {
    ...
    function getParticleColor() {
        ...
        return isLight ? '0,136,187' : '0,245,255';
    }
        ...
        for (let i = 0; i < len; i++) {
            ...
            for (let j = i + 1; j < len; j++) {
                ...
                if (dist < lineDist * lineDist) {
                    ...
                    ctx.moveTo(p.x, p.y);
                    ctx.lineTo(q.x, q.y);
                    ctx.strokeStyle = 'rgba(' + rgb + ',' + (0.15 * (1 - Math.sqrt(dist) / lineDist)) + ')';
                    ctx.lineWidth = 0.5;
                    ctx.stroke();
                }
            }
    ...
    function initTheme() {
        ...
        // Sync button state with current theme (set by theme.js before paint)
